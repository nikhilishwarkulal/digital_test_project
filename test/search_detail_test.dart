...
void main() {
  _injectRegistry();
  testWidgets('Search Detail Test', (WidgetTester tester) async {
    ...
    await tester.tap(find.byType(SearchListingCell).first);
    await tester.pumpAndSettle();

    ///Verify if detail screen is opened
    expect(find.byType(SearchDetailScreen), findsOneWidget);

    ///Verify if detail screen is opened first item in search list which
    ///is from api first item
    expect(find.text("Texas Rangers at Minnesota Twins"), findsNothing);
    expect(find.text("Texas Rangers at Minnesota Twins two"), findsOneWidget);

    ///Verify if the city is displayed in detail screen.
    ...
    ///Verify if detail screen is not available anymore
    expect(find.byType(SearchDetailScreen), findsNothing);
  });
}
